...
import {
  ...
  deleteTestChoiceQuestion, searchTestChoiceQuestion,
  updateTestChoiceQuestion
} from "@/services/ant-design-pro/choiceQuestion";
import {getChoiceQuestionTestIdTitleVO} from "@/services/ant-design-pro/test";
 ...
 * @param fields
 */
const handleUpdate = async (fields: API.ChoiceQuestionTestUpdateRequest) => {
  const hide = message.loading('修改中');
  try {
    await updateTestChoiceQuestion({
      id: fields.id,
      score: fields.score,
    });
    hide();
...
const TableList: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<API.ChoiceQuestionTestAdminVo>();
  ...
  const columns: ProColumns<API.ChoiceQuestionTestAdminVo>[] = [
    {
      title: 'ID',
      ...
      render: (text, record, _, action) => [
        ...
        <a
          ...
          onClick={async () => {
            ...
            const res = await deleteTestChoiceQuestion(record.id);
            if (res.code === 0) {
  ...
  return (
    <PageContainer>
      <ProTable<API.ChoiceQuestionTestAdminVo, API.PageParams>
        headerTitle={'题目信息'}
        actionRef={actionRef}
        rowKey="key"
        search={{
        ...
        toolBarRender={() => [
          <Button
            ...
            onClick={() => {
              ...
              getChoiceQuestionTestIdTitleVO().then(res => {
                  const  data = []
                  // @ts-ignore
        ...
        request={
          async (params = {}, sort, filter) => {
            ...
            // @ts-ignore
            const res = await searchTestChoiceQuestion(param);
            return {
              data: res.data,
            }
          }
      ...
      <ModalForm
        ...
        onFinish={async (value) => {
          const success = await handleAdd(value as API.ChoiceQuestionTestAddRequest);
          if (success) {
      ...
      >
        {currentRow?.id&& (
          <ProDescriptions<API.TrueOrFalseAdminVo>
            ...
            columns={columns as ProDescriptionsItemProps<API.ChoiceQuestionTestAdminVo>[]}
          />
        )}
      </Drawer>
    </PageContainer>
